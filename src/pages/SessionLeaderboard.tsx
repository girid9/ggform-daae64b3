...
const SessionLeaderboard = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: sessionRow, error: sessionErr } = await supabase
        .from("quiz_sessions")
        .select("id, name, session_code, question_ids, created_at")
        .eq("session_code", code)
        .maybeSingle();

      if (sessionErr || !sessionRow) {
        setError("This quiz leaderboard link is invalid or has expired.");
        setLoading(false);
        return;
      }
  ...
  const leaders: LeaderEntry[] = useMemo(() => {
    ...
    attempts.forEach((a) => {
      ...
      if (!existing) {
        ...
        if (new Date(a.created_at) > new Date(existing.lastAt)) {
          existing.lastAt = a.created_at;
        }
      }
    });
  ...
  const handleShare = async () => {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: session?.name ?? "Quiz Leaderboard", url });
      } else {
    ...
    <div className="page-bg min-h-screen px-4 py-6 sm:px-6">
      <div className="mx-auto max-w-3xl">
        ...
        <section className="hero-shell mt-5 p-6 sm:p-8">
          <span className="chip-primary">Quiz leaderboard</span>
          ...
          <p className="mt-3 text-sm font-mono text-muted-foreground">
            Code: {session.session_code} • {session.question_ids.length} questions
          </p>
          ...
          {leaders.length === 0 ? (
            <div className="mt-6 rounded-[22px] border border-dashed border-border/70 p-8 text-center">
              <p className="text-sm text-muted-foreground">
                No one has taken this quiz yet. Share the quiz link to get started.
              </p>
            </div>
          ) : (
